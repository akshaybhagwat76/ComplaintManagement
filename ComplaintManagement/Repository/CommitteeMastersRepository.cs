...
namespace ComplaintManagement.Repository
{
    public class CommitteeMastersRepository
    {
        ...
        public CommitteeMasterVM AddOrUpdate(CommitteeMasterVM CommitteeVM)
        {
                ...
                if (!string.IsNullOrEmpty(sid))
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            new EmployeeComplaintHistoryRepository().ErrorLogHistory(null, ex.Message.ToString(), "Committee Master");
            ...
            {
                new EmployeeComplaintHistoryRepository().ErrorLogHistory(null, string.Join("\n", dve.EntityValidationErrors.SelectMany(x => x.ValidationErrors).Select(y => y.ErrorMessage)), "Committee Master");
