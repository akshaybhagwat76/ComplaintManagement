...
namespace ComplaintManagement.Repository
{
    public class LocationMastersRepository
    {
        ...
        public LocationMasterVM AddOrUpdate(LocationMasterVM LocationVM)
        {
                ...
                if (!string.IsNullOrEmpty(sid))
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            new EmployeeComplaintHistoryRepository().ErrorLogHistory(null, ex.Message.ToString(), "Location Master");
            ...
            {
                new EmployeeComplaintHistoryRepository().ErrorLogHistory(null, string.Join("\n", dve.EntityValidationErrors.SelectMany(x => x.ValidationErrors).Select(y => y.ErrorMessage)), "Location Master");
