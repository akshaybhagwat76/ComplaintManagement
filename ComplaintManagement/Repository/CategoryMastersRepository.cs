...
namespace ComplaintManagement.Repository
{
    public class CategoryMastersRepository
    {
        ...
        public CategoryMasterVM AddOrUpdate(CategoryMasterVM categoryVM)
        {
                ...
                if (!string.IsNullOrEmpty(sid))
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            new EmployeeComplaintHistoryRepository().ErrorLogHistory(null, ex.Message.ToString(), "Category Master");
            ...
            {
                new EmployeeComplaintHistoryRepository().ErrorLogHistory(null, string.Join("\n", dve.EntityValidationErrors.SelectMany(x => x.ValidationErrors).Select(y => y.ErrorMessage)), "Category Master");
        ...
        public List<EmployeeCompliantMasterVM> GetAllCatReport(string range, int losid)
        {
            List<EmployeeCompliantMasterVM> WorkFlowList = new List<EmployeeCompliantMasterVM>();
            List<EmployeeComplaintMaster> WorkFlows = new List<EmployeeComplaintMaster>();
            List<EmployeeComplaintWorkFlowVM> WorkFlowList1 = new List<EmployeeComplaintWorkFlowVM>();
            List<EmployeeComplaintWorkFlow> WorkFlows1 = new List<EmployeeComplaintWorkFlow>();
            List<CategoryMasterVM> Categories = new List<CategoryMasterVM>();
            if (!string.IsNullOrEmpty(range))
            {
                string[] dates = range.Split(',');
                DateTime fromDate = Convert.ToDateTime(dates[0]);
                ...
                using (DB_A6A061_complaintuserEntities db = new DB_A6A061_complaintuserEntities())
                {
                    try
                    {
                        LOSMasterRepository ls = new LOSMasterRepository();
                        var Categori = ls.GetAllCategoryReport();
                        var listoflists = Categori.GroupBy(x => x.Id).Select(y => y.First());
                        Categories = listoflists.ToList();

                        if (Categories != null)
                        {
                            foreach (var items in Categories)
                            {
                                WorkFlows.AddRange(db.EmployeeComplaintMasters.Where(i => i.IsActive && i.CategoryId == items.Id && i.CreatedDate >= fromDate && i.CreatedDate <= toDate).ToList().OrderByDescending(x => x.CreatedDate).OrderByDescending(x => x.Id).ToList());
                            }
                        }
